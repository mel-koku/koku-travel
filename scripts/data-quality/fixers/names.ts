...
export const nameFixer: Fixer = {
  handles: ['EVENT_NAME_MISMATCH', 'ALL_CAPS_NAME', 'BAD_NAME_START', 'NAME_ID_MISMATCH'],
  ...
  async fix(issue: Issue, ctx: FixerContext): Promise<FixResult> {
    ...
    // For EVENT_NAME_MISMATCH, try Google Places lookup
    if (issue.type === 'EVENT_NAME_MISMATCH' && ctx.googleApiKey) {
