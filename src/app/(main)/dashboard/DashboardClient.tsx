...
export function DashboardClient({ initialAuthUser, content }: DashboardClientProps) {
    ...
    return (
      <div className="flex min-h-[100dvh] items-center justify-center bg-surface">
        <div className="text-center">
          ...
          <p className="text-sm text-foreground-secondary">Pulling up your trips\u2026</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-[100dvh] bg-background">
      ...
      {!isAuthenticated && signInPrompts.shouldShowReturnVisit && trips.length > 0 && (
        <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 mt-6">
          <GuestSignInPrompt
            onDismiss={() => signInPrompts.dismiss("return-visit")}
          />
        </div>
      )}
          ...
          <div className="pointer-events-auto flex flex-col gap-3 rounded-lg border border-border bg-background p-4 shadow-[var(--shadow-elevated)] ring-1 ring-brand-primary/20" role="alert">
            ...
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={handleUndo}
                className="rounded-lg bg-brand-primary px-3 py-1.5 text-xs font-semibold text-white shadow-[var(--shadow-sm)] transition hover:bg-brand-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-2"
              >
