...
export const GET = withApiHandler(
  async (_request, { context }) => {
    ...
    // Try server-side aggregation via RPC (single query, no client-side processing)
    const { data: rpcResult, error: rpcError } = await supabase.rpc("get_filter_metadata");

    let response: FilterMetadata;

    if (!rpcError && rpcResult) {
      // RPC succeeded: use pre-aggregated data directly
      response = rpcResult as FilterMetadata;
    } else {
      // Fallback: client-side aggregation (RPC function may not be deployed yet)
      if (rpcError) {
        logger.warn("get_filter_metadata RPC unavailable, falling back to client-side aggregation", {
      ...
      response = {
        cities: Array.from(cityMap.entries()).map(([value, count]) => ({ value, label: value, count })).sort((a, b) => a.label.localeCompare(b.label)),
        categories: Array.from(categoryMap.entries()).map(([value, count]) => ({ value, label: value, count })).sort((a, b) => a.label.localeCompare(b.label)),
        regions: Array.from(regionMap.entries()).map(([value, count]) => ({ value, label: value, count })).sort((a, b) => a.label.localeCompare(b.label)),
        prefectures: Array.from(prefectureMap.entries()).map(([value, count]) => ({ value, label: value, count })).sort((a, b) => a.label.localeCompare(b.label)),
        neighborhoods: Array.from(neighborhoodMap.entries()).map(([value, count]) => ({ value, label: value, count })).sort((a, b) => a.label.localeCompare(b.label)),
      };
    }
