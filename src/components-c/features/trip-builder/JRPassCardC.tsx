...
export function JRPassCardC({ duration, cities }: JRPassCardCProps) {
            ...
            {regionalPasses.map((rp) => (
              <div key={rp.pass.id} className="flex items-start justify-between text-xs">
                <div className="min-w-0 flex-1">
                  ...
                  {rp.pass.notes && (
                    <p className="mt-0.5 line-clamp-1 text-[10px]" style={{ color: "var(--muted-foreground)" }}>
                      {rp.pass.notes}
                    </p>
                  )}
                </div>
                <span className="ml-2 shrink-0 tabular-nums" style={{ color: "var(--muted-foreground)" }}>
