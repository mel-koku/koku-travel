...
type PlaceDetailPanelCProps = {
  location: Location;
  onClose: () => void;
};

export function PlaceDetailPanelC({ location, onClose }: PlaceDetailPanelCProps) {
  ...
  return (
    <>
      ...
      <motion.div
        data-lenis-prevent
        className="fixed inset-0 z-50 bg-[var(--background)] flex flex-col overflow-y-auto overscroll-contain sm:inset-y-0 sm:left-auto sm:right-0 sm:w-[560px] sm:max-w-[90vw] sm:border-l sm:border-[var(--border)]"
        initial={{ x: "100%" }}
        animate={{ x: 0 }}
        exit={{ x: "100%" }}
        transition={{ duration: DURATION_FAST, ease: cEase }}
      ...
      >
        ...
        <button
          onClick={onClose}
          title="Close"
          className="absolute top-3 right-3 z-10 flex h-11 w-11 items-center justify-center bg-white/80 hover:bg-white transition"
        ...
        {/* Photo strip */}
        {allPhotos.length > 1 && (
          <div className="flex gap-px overflow-x-auto snap-x snap-mandatory scrollbar-hide px-5 -mt-5 relative z-[1]">
            {allPhotos.map((src, i) => (
              <button
                key={src}
                type="button"
                ...
                className={`relative h-11 w-16 shrink-0 snap-start overflow-hidden transition-all duration-200 ${
        ...
        <div className="flex-1 px-5 pt-5 pb-8 space-y-6">
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 px-7 py-2.5 text-[11px] font-bold uppercase tracking-[0.15em] transition-all hover:scale-[1.02] active:scale-[0.98] min-h-[44px] ${
              isSaved
          ...
          {(summary || description) && (
            <section className="space-y-1.5">
              <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--muted-foreground)]">
                Overview
              </h3>
              {summary && (
          ...
          {people.length > 0 && (
            <section className="space-y-3">
              <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--primary)]">
              ...
              {people.map((person) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {person.bio && (
                      <p className="mt-1 text-xs leading-relaxed text-[var(--muted-foreground)] line-clamp-3">
                        {person.bio}
                      </p>
                    )}
            ...
            <section className="space-y-2.5">
              ...
              <div className="space-y-2">
                {tips.map((tip) => (
                  <div
                    key={tip.id}
                    className="flex gap-2.5 border border-[var(--border)] p-4"
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-[var(--muted-foreground)] mt-0.5 leading-relaxed">{tip.summary}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {status === "success" && (
            <section className="space-y-2">
              ...
              <div className="border border-[var(--border)] p-3.5">
                {hasOpeningHours ? (
                  ...
                  <p className="text-xs text-[var(--muted-foreground)]">Open 24 hours or hours not listed</p>
                )}
              </div>
            </section>
          )}
            ...
            <section className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {accessibilityBadges.map((badge) => (
                  <span
                    key={badge.key}
                    className="inline-flex items-center gap-1 border border-[var(--border)] px-2.5 py-1 text-xs text-[var(--muted-foreground)]"
                  >
            ...
            <section className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {goodForPills.map((pill) => (
                  <span
                    key={pill.key}
                    className="border border-[var(--border)] px-3 py-1 text-sm text-[var(--foreground)]"
                  >
                    {pill.label}
                  </span>
                ))}
          ...
          {details?.formattedAddress && (
            <section className="space-y-1">
              ...
              <p className="text-sm text-[var(--foreground)]">{details.formattedAddress}</p>
            </section>
          )}
            ...
            <section className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm">
                ...
                {details!.internationalPhoneNumber && (
                  <span className="text-[var(--foreground)]">{details!.internationalPhoneNumber}</span>
                )}
