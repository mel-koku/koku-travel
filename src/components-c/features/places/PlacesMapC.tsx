...
// C cluster palette: vermillion steps
const CLUSTER_SMALL = "#e63312";
const CLUSTER_MEDIUM = "#cc2d10";
const CLUSTER_LARGE = "#a3240d";
...
const SOURCE_ID = "places-locations-c";
const CLUSTER_LAYER = "places-clusters-c";
const CLUSTER_COUNT_LAYER = "places-cluster-count-c";
const UNCLUSTERED_LAYER = "places-unclustered-point-c";
const UNCLUSTERED_LABEL_LAYER = "places-unclustered-label-c";

export function PlacesMapC({
  locations,
  onBoundsChange,
  onLocationClick,
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      // Cluster circles: vermillion stepped palette
      map.addLayer({
        id: CLUSTER_LAYER,
        type: "circle",
        source: SOURCE_ID,
      ...
      // Individual points: square-ish markers via larger circle
      map.addLayer({
  ...
  if (!accessToken || !mapboxEnabled) {
    return (
      <div className="flex h-full w-full items-center justify-center border border-[var(--border)] bg-[var(--background)] p-4">
  ...
  return (
    <div className="h-full w-full bg-[var(--background)]" style={{ position: "relative" }}>
      {!mapReady && (
        <div className="flex items-center justify-center bg-[var(--background)]" style={{ position: "absolute", inset: 0, zIndex: 1 }}>
      ...
      {showResetButton && (
        <button
          ...
          className="flex items-center gap-1.5 bg-[var(--foreground)] px-4 py-2 text-[11px] font-bold uppercase tracking-[0.1em] text-[var(--background)] transition-colors hover:opacity-90 active:scale-[0.98]"
