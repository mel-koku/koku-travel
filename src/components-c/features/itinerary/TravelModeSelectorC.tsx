...
export function TravelModeSelectorC({
  currentMode,
  durationMinutes,
  departureTime,
  origin,
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="inline-flex items-center gap-1.5 text-sm transition-colors hover:text-[var(--primary)] active:scale-[0.98]"
      ...
      >
        ...
        <span className="text-[10px] uppercase tracking-[0.1em]">{currentOption?.label}</span>
        <ChevronDown
      ...
      {isOpen && (
        <div
          className="absolute left-0 top-full z-50 mt-1 w-[min(14rem,90vw)] border p-1"
          style={{
          ...
          {TRAVEL_MODES.map((option) => {
            ...
            return (
              <button
                ...
                className="flex w-full items-center justify-between gap-2 px-2 py-1.5 text-left text-xs transition-colors hover:bg-[color-mix(in_srgb,var(--foreground)_5%,transparent)] active:scale-[0.98]"
                style={{
