...
type ExperienceHighlightCProps = {
  value: {
...
function getBorderColor(highlightType: string): string {
  switch (highlightType) {
    case "sensory":
      return "var(--success, #3da193)";
    case "practical":
...
export function ExperienceHighlightBlockC({ value }: ExperienceHighlightCProps) {
  const config = HIGHLIGHT_CONFIG[value.highlightType] || HIGHLIGHT_CONFIG.key_moment;
  const borderColor = getBorderColor(value.highlightType);
