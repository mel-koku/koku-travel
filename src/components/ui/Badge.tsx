...
const badgeVariants = cva(
  "inline-flex items-center rounded-md px-3 py-1 text-xs font-semibold uppercase tracking-wide transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
  {
    variants: {
      variant: {
        default:
...
function Badge({ className, variant = "soft", tone = "brand", ...props }: BadgeProps) {
  ...
  if (tone && (variant === "solid" || variant === "soft" || variant === "outline")) {
    return (
      <div
        className={cn(
          "inline-flex items-center rounded-md px-3 py-1 text-xs font-semibold uppercase tracking-wide transition-colors",
