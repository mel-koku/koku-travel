  ...
  return (
    <>
      ...
      <div
        ref={sheetRef}
        className={cn(
          "fixed inset-x-0 bottom-0 z-50 flex flex-col rounded-t-lg bg-background pb-[env(safe-area-inset-bottom)] shadow-[var(--shadow-elevated)] transition-transform lg:hidden",
...
export function PreviewToggleButton({ onClick, isOpen }: PreviewToggleButtonProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="fixed bottom-[calc(5rem+env(safe-area-inset-bottom))] right-4 z-30 flex items-center gap-2 rounded-full bg-brand-primary px-4 py-2.5 text-sm font-medium text-white shadow-[var(--shadow-elevated)] transition hover:bg-brand-primary/90 active:scale-[0.98] lg:hidden"
    >
