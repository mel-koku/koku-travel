...
export type FavoritesInTripPreviewProps = {
  selectedCities: string[] | undefined;
};

export function FavoritesInTripPreview({
  selectedCities,
}: FavoritesInTripPreviewProps) {
  const { favorites } = useAppState();
  const { data: locations, isLoading } = useWishlistLocations(favorites);
    ...
    return (
      <div className="rounded-lg border border-border bg-surface p-4">
        <div className="flex items-center gap-2 text-sm text-stone">
          ...
          <span>Checking your favorites...</span>
        </div>
      </div>
    );
  }
