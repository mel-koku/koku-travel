        ...
        <div className="absolute left-0 top-full z-30 mt-1.5 w-52 rounded-xl border border-border bg-surface p-3 shadow-lg">
          ...
          <div className="flex flex-wrap gap-1.5">
            {PRESETS.map((preset) => (
              <button
                ...
                className="rounded-lg bg-background px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-brand-primary/10 hover:text-brand-primary"
              >
                {preset.label}
              </button>
            ))}
          ...
          <div className="mt-2.5 flex items-center gap-1.5">
            <input
              ...
              className="h-8 w-16 rounded-lg border border-border bg-background px-2 text-center text-xs text-foreground placeholder:text-stone focus:outline-none focus:ring-1 focus:ring-brand-primary"
            />
            <button
              type="button"
              onClick={applyCustom}
              disabled={!customMinutes || parseInt(customMinutes, 10) < 1}
              className="rounded-lg bg-brand-primary/10 px-3 py-1.5 text-xs font-medium text-brand-primary transition-colors hover:bg-brand-primary/20 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Apply
            </button>
          </div>
