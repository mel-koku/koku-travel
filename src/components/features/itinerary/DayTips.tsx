...
export function DayTips({ day, tripStartDate, dayIndex, className, embedded, onTipCount, nextDayActivities, isFirstTimeVisitor, hasLuggagePrompt }: DayTipsProps) {
  ...
  const proTips = useMemo<DisplayTip[]>(() => {
    ...
    // Last train warning — check if any evening return transit departs after last train
    if (day.cityId) {
      ...
      if (lastTrainTime) {
        const hasLateTransit = activities.some((a) => {
          ...
          return depMin > lastTrainTime - 30; // Warn 30min before last train
        });
        if (hasLateTransit) {
          tips.push({
            id: "pro-last-train",
    ...
    // Cash-only warning — check next day's activities for cash-only locations
    ...
    // Seasonal food tip — show on days with restaurant activities
    ...
    // Festival tip — show when there's a festival today in this city
    if (day.cityId) {
      const dayOfMonth = dayDate.getDate();
