      ...
      <div className="relative z-10 mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {locations.map((location) => (
          ...
          >
            ...
            <p className="px-5 pb-5 pt-4 text-sm text-stone-700">{getHeroSummary(location)}</p>
          </article>
        ))}
      </div>
      <div className="absolute -top-20 right-[-10%] h-64 w-64 rounded-full bg-emerald-200/20 blur-3xl" aria-hidden="true" />
      <div className="absolute -bottom-24 left-[-6%] h-56 w-56 rounded-full bg-stone-300/15 blur-3xl" aria-hidden="true" />
    </section>
  );
}
