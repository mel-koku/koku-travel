...
/**
 * Normalize operating_hours coming from the DB.
 *
 ...
 *
 * Downstream consumers (getOpenStatus, getOperatingPeriodForDay) only
 * understand the textual shape, so any location in the v1 shape appears
 * permanently closed. Convert v1 to textual at the boundary.
 */
function normalizeOperatingHours(raw: unknown): LocationOperatingHours | undefined {
