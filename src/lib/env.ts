...
function validateEnv(): EnvConfig {
  ...
  try {
    ...
    // If strict validation fails, log and fall back to lenient mode
    // This prevents the app from crashing if variables are missing
    logger.error("Environment validation failed, falling back to lenient mode", error instanceof Error ? error : new Error(String(error)));
    return createLenientConfig();
  }
}
...
try {
  ...
  // Final fallback: if validation itself throws, use lenient config
  logger.error("Environment validation error, using lenient config", error instanceof Error ? error : new Error(String(error)));
  envConfig = createLenientConfig();
}
