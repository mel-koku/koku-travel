...
type DayRefinementButtonsBProps = {
  dayIndex: number;
  tripId: string;
    ...
    async (type: RefinementType) => {
      ...
      try {
        const response = await fetch("/api/itinerary/refine", {
          ...
          body: JSON.stringify({ tripId, dayIndex, refinementType: type, builderData, itinerary }),
        });

        if (!response.ok) {
          const error = await response
            .json()
            .catch(() => ({ message: "Couldn't adjust this day — try again." }));
        ...
        setRefinementError(
          error instanceof Error ? error.message : "Couldn't adjust this day — try again.",
        );
      } finally {
        setIsRefining(false);
      }
