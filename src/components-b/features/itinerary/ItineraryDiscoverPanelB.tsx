...
type ItineraryDiscoverPanelBProps = {
...
export function ItineraryDiscoverPanelB({
  locations,
  isLoading,
  category,
  onCategoryChange,
  ...
  geoLoading,
}: ItineraryDiscoverPanelBProps) {
  const searchInputRef = useRef<HTMLInputElement>(null);

  return (
    <div className="flex h-full flex-col">
      {/* Filters */}
      <div className="space-y-2.5 border-b px-5 pb-3 pt-3" style={{ borderColor: "var(--border)" }}>
        {/* Category chips */}
        <div className="flex gap-1.5 overflow-x-auto overscroll-contain scrollbar-hide">
          {CATEGORY_CHIPS.map((chip) => (
            <button
              ...
              style={{
                backgroundColor: category === chip.id ? "var(--primary)" : "var(--surface)",
                color: category === chip.id ? "white" : "var(--muted-foreground)",
              }}
            >
              {chip.label}
            </button>
        ...
        <div className="flex items-center gap-2">
          <div className="relative min-w-0 flex-1">
            ...
            <input
              ...
              className="h-9 w-full rounded-xl border pl-8 pr-3 text-sm focus:outline-none"
              style={{
          ...
          <button
            ...
            style={{
              ...
              backgroundColor: openNow
                ...
                : "transparent",
              color: openNow ? "var(--primary)" : "var(--muted-foreground)",
            }}
          >
            <span
              className="inline-block h-1.5 w-1.5 rounded-full"
              style={{ backgroundColor: openNow ? "var(--primary)" : "var(--muted-foreground)" }}
            />
            Open now
          </button>
        </div>
            ...
            {locations.map((loc) => {
              ...
              return (
                <motion.div
                  ...
                  className="group relative cursor-pointer rounded-2xl p-3 transition-all"
                  style={{
                    ...
                    <div className="shrink-0">
                      {isUsed ? (
                        ...
                        <button
                          ...
                          className="rounded-xl px-2.5 py-1.5 text-[11px] font-medium text-white transition-shadow hover:shadow-[var(--shadow-elevated)] active:scale-[0.98]"
