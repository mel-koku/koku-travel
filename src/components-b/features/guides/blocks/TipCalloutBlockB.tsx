...
function getAccent(tipType: string): string {
  switch (tipType) {
    case "budget":
      return "var(--warning)";
    case "warning":
...
export function TipCalloutBlockB({ value }: TipCalloutBProps) {
  const config = TIP_CONFIG[value.tipType] || TIP_CONFIG.pro_tip;
  const accent = getAccent(value.tipType);
